...
const CustomHeading: React.FC<HeadingProps & { level: number }> = ({
  level,
  children,
  className,
}) => {
  ...
  const headingClass =
    DEFAULT_MARKDOWN_THEME.typography.headings[
...
const CustomParagraph: React.FC<ParagraphProps & { node?: unknown }> = ({
  children,
  className,
}) => {
...
const CustomList: React.FC<ListProps & { ordered?: boolean }> = ({
  ordered,
  children,
  className,
}) => {
...
// Simple and clean table components
    ...
    const components = useMemo(() => {
      const memoizedComponents = {
        ...
        p: ({ children, node, ...props }: any) => (
          <CustomParagraph node={node} {...props}>
            {children}
          </CustomParagraph>
        ),
