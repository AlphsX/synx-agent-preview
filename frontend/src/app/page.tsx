...
- PostgreSQL with pgvector (for knowledge search)`;
  ...
  useEffect(() => {
    ...
    if (!browserInfo.isSupported) {
      // Don't initialize speech recognition for unsupported browsers
      return;
    }
    ...
    // Use browser detection for speech recognition support
    const speechSupport = checkSpeechRecognitionSupport(browserInfo);

    if (!speechSupport.supported) {
      // Don't show error immediately on mobile - wait for user interaction
      if (!isMobile) {
    ...
    // Create new SpeechRecognition instance with mobile optimizations
                ...
                <form onSubmit={handleSubmit} className="relative">
                  ...
                  <div className="flex items-end space-x-2 bg-transparent rounded-3xl p-2">
                    ...
                    <div className="flex-1 relative">
                      {/* Mobile voice status indicator */}
                      {isListening && (
                        <div className="mb-2 flex items-center justify-center md:hidden">
                          <div className="flex items-center space-x-2 bg-red-500/10 dark:bg-red-500/20 text-red-600 dark:text-red-400 px-3 py-2 rounded-full text-sm font-medium">
                      ...
                      <div className="relative bg-transparent rounded-3xl border border-gray-900/20 dark:border-gray-100/20">
                        ...
                        <textarea
                          ...
                          onChange={(e) => {
                            ...
                            // Toggle tools dropdown when "/" is typed as first character
                            if (newValue === "/" && !isToolsDropdownOpen) {
                              // Open tools dropdown
                              const plusButton = document.querySelector(
                                "[data-plus-button]"
                              ) as HTMLButtonElement;
                              ...
                              // Close tools dropdown when "/" is deleted
                              const plusButton = document.querySelector(
                                "[data-plus-button]"
                              ) as HTMLButtonElement;
                              if (plusButton) {
                          ...
                          className={`w-full resize-none bg-transparent px-14 py-3 pr-14 text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none leading-relaxed font-medium chat-input ${
                        ...
                        <div className="absolute right-2 bottom-2 flex items-center space-x-1">
                          {/* Enhanced Voice input button with mobile optimization */}
                          <button
                            type="button"
